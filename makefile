...
migrateup:
	migrate -path db/migrations -database "postgresql://${DB_USER}:${DB_PASS}@localhost:${DB_PORT}/${DB_NAME}?sslmode=${DB_SSL_MODE}" -verbose up

migratedown:
	migrate -path db/migrations -database "postgresql://${DB_USER}:${DB_PASS}@localhost:${DB_PORT}/${DB_NAME}?sslmode=${DB_SSL_MODE}" -verbose down
